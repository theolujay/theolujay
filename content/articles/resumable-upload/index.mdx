...
The main lesson is that resumability depends on a position both sides can trust. Chunking makes retries smaller; staging, commit ordering, and locking keep that position consistent.

For production uploads at Retreev, I'd need to address requirements beyond this learning project:

- **Access and resource limits:** Add authentication, upload ownership, appropriate timeouts, and cleanup for abandoned uploads. *Moove* currently allows any browser origin and has no authentication.
- **Storage and durability:** Replace the JSON database and process-local locks with storage and coordination suited to multiple server instances. Test failure recovery before increasing the 10 MiB limit.
- **Protocol coverage:** Tighten metadata validation and run broader conformance checks. *Moove* does not implement concatenation, which combines separately uploaded parts into a final upload.

For that work, I'd likely use [tusd](https://github.com/tus/tusd), the official Go server implementation, rather than turn *Moove* into a production service. Building my own was a way to understand the protocol and the failure cases I'd need to evaluate in any implementation, not a reason to maintain every part myself.

The [repository](https://github.com/theolujay/resumable-upload) contains the server, tests, and setup instructions, including a small upload you can try with `curl`. To start the server locally, run this from the repository root:

```sh
go run ./cmd/cli -port 4000 -log-level debug
```

I started with “how do I avoid sending the whole file again?” The more useful question turned out to be “what has the server actually committed?” Once that answer is reliable, the client has somewhere safe to continue.
